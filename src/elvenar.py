...
from src.Utils import finish_sound, get_time, calc_stocktimes, calc_goodtimes
# from src.Production import Production
# from Gui import Gui, load_app, ex
from utils.helpers import say
...
class Elvenar:
    def __init__(self, load_gui=False):
    ...
    def farm(self, i=0):
        ...
        while True:
            self.Mouse.press(*s)
            for p in np.linspace(s, e, 8):
                self.Mouse.move(*p, wait=.1)
            self.Mouse.release(*e)
            sleep(2)
            self.Mouse.left_click(*s, wait=.5)
            self.Mouse.left_click(*w_pos[i])
            sleep(t[i] * 60 - 5)
            say('10 seconds left!')
            sleep(10)
            print(f'\r{j}', end='')
    ...
    def plant_stock_loop(self, t=15, farm=True):
        while not input('Wanna continue? '):
